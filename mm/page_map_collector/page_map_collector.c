...
MODULE_DESCRIPTION("Lightweight page access pair collector (mark_page_accessed + handle_mm_fault)");
MODULE_VERSION("0.1");
...
MODULE_PARM_DESC(sample_rate, "Sampling rate: keep 1 every N events (default 1)");
...
struct recent_buf {
    ...
    unsigned int count; // number of valid items currently in buffer (<= buf_len)
    spinlock_t lock;
};
...
static unsigned int process_interval_ms = 50; // periodic worker interval fallback
...
{
    return bucket & (bucket_lock_count - 1); // bucket_lock_count must be power of two
}
...
static unsigned int snapshot_percpu_buffer(int cpu, struct page_event *out, unsigned int max_items)
{
    struct recent_buf *rb = per_cpu_ptr(pcpu_bufs, cpu);
    unsigned long flags;
...
static void ensure_bucket_locks(unsigned int buckets)
{
    ...
    unsigned int p = 1;
    while (p < 256 && p < buckets) p <<= 1;
    bucket_lock_count = p;
    bucket_locks = kcalloc(bucket_lock_count, sizeof(spinlock_t), GFP_KERNEL);
    if (!bucket_locks) bucket_lock_count = 1;
    else {
        unsigned int i;
...
static void process_all_buffers(struct work_struct *work)
{
    ...
    snapshot = kmalloc_array(max_items, sizeof(struct page_event), GFP_KERNEL);
    if (!snapshot) return;

    // for each CPU, snapshot buffer and find pairs within delta_ns
    for_each_possible_cpu(cpu) {
        unsigned int cnt = snapshot_percpu_buffer(cpu, snapshot, max_items);
        if (cnt == 0) continue;
        ...
        for (i = 0; i < cnt; ++i) {
            ...
            for (j = i + 1; j < cnt; ++j) {
                ...
                if (ts_j < ts_i) continue;
                if (ts_j - ts_i > delta_ns) break;
...
static int kp_pre_handler(struct kprobe *p, struct pt_regs *regs)
{
...
#ifdef CONFIG_X86_64
    if (p->symbol_name && strcmp(p->symbol_name, "mark_page_accessed") == 0) {
    ...
    } else if (p->symbol_name && strcmp(p->symbol_name, "handle_mm_fault") == 0) {
...
static int proc_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "# stats: total_events=%lld total_pairs=%lld dropped=%lld\n",
...
static int __init pm_init(void)
{
    ...
    if (buf_len > 65536) buf_len = 65536;
