...
#define MODULE_NAME "page_window"
...
static int proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(
		m,
		"# params: on_ms=%u off_ms=%u buf_len=%u pair_hash_bits=%u max_report=%u\n",
		on_ms, off_ms, buf_len, pair_hash_bits, max_report);
...
static int __init pw_init(void)
{
	...
	pr_info("%s: init (on_ms=%u off_ms=%u buf_len=%u hash_bits=%u)\n",
		MODULE_NAME, on_ms, off_ms, buf_len, pair_hash_bits);
