...
MODULE_DESCRIPTION("Windowed collector for page-access co-occurrence (mark_page_accessed + handle_mm_fault)");
MODULE_VERSION("0.2");
...
MODULE_PARM_DESC(buf_len, "Per-CPU buffer length (max events per window). Default 16k");

static unsigned int pair_hash_bits = 12; // hashtable size 2^bits
module_param(pair_hash_bits, uint, 0644);
MODULE_PARM_DESC(pair_hash_bits, "Hash bits for pair table (2^bits buckets). Default 12");
 ...
 */
static unsigned int snapshot_percpu_to_array(int cpu, struct page_event *out_buf, unsigned int max_items)
{
    struct recent_buf *rb = per_cpu_ptr(pcpu_bufs, cpu);
    unsigned long flags;
...
static void process_window_work(struct work_struct *work)
{
    ...
    for_each_possible_cpu(cpu) {
        unsigned int cnt = snapshot_percpu_to_array(cpu, snap, max_items);
        if (cnt == 0) continue;
...
static int __init pw_init(void)
{
    ...
    if (buf_len < 128) buf_len = 128;
    if (buf_len > 131072) buf_len = 131072;
    ...
    // init timers
    kt_on = ktime_set(0, on_ms * MS_TO_NS(1));   // ktime_set secs=nsec? better compute below
